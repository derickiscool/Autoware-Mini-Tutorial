...
class YoloTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        ...
        if len(local_path_msg.waypoints) > 0:
            local_path_xyz = [(wp.position.x, wp.position.y, wp.position.z) for wp in local_path_msg.waypoints]
        ...
        if stop_line_ids_on_path:
            ...
            try:
                transform = self.tf_buffer.lookup_transform(transform_to_frame, transform_from_frame,
                                                            image_time_stamp, rospy.Duration(self.transform_timeout))
